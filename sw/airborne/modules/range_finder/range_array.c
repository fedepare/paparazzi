 ...
 */
/**
 * @file "modules/range_finder/range_array.c"
 * @author Kirk Scheper
 * Reads an array of range readings from an mcu
 */

#include "modules/range_finder/range_array.h"
...
static struct range_array_t range_array = {
  .device = (&((RANGE_ARRY_PORT).device)),
  .msg_available = false
};
static uint8_t ra_msg_buf[128]  __attribute__((aligned));   ///< The message buffer for the range array
...
static inline void mag_pitot_parse_msg(void)
{
  ...
  case DL_IMCU_REMOTE_MAG: {
    ...
    // Rotate the magneto
    struct Int32RMat *imu_to_mag_rmat = orientationGetRMat_i(&range_array.imu_to_mag);
...
void mag_pitot_event() {
  ...
  pprz_check_and_parse(range_array.device, &range_array.transport, mp_msg_buf, &range_array.msg_available);

  // If we have a message we should parse it
  if (range_array.msg_available) {
    mag_pitot_parse_msg();
    range_array.msg_available = false;
  }
}
