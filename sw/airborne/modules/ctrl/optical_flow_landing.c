...
float dstate;
float dt;
int vision_message_time;
int previous_message_time;
int landing;
float previous_err;
...
static float get_cov(float *a, float *b, int n_elements);
static float get_mean_array(float *a, int n_elements);

// common functions for different landing strategies:
static void set_cov_div(int32_t thrust);
static int32_t PID_divergence_control(float divergence_setpoint, float P, float I, float D, float *err);
static void update_errors(float error);
static void final_landing_procedure(void);
  ...
  if (of_landing_ctrl.VISION_METHOD == 0) {
    ...
    if (dt > 0.0001f) {
      ...
      if (of_landing_ctrl.agl_lp > 0.0001f) {
        ...
        divergence_vision_dt = divergence_vision;//(divergence_vision / dt);
    ...
    if (vision_message_time > previous_message_time && dt > 1E-5 && ind_hist > 1) {
      ...
      float new_divergence = divergence_vision; //(divergence_vision * div_factor) / dt;
      ...
      previous_message_time = vision_message_time;
      dt = 0.0f;
    } else {
      ...
      if (ind_hist <= 1) {
        ...
        dt = 0.0f;
        if (in_flight) {
          stabilization_cmd[COMMAND_THRUST] = of_landing_ctrl.nominal_thrust * MAX_PPRZ;
        }
      }
    ...
    ***********/
    ...
    if (!landing) {
      ...
      if (module_active_time_sec < 2.5f) {
        thrust = of_landing_ctrl.nominal_thrust * MAX_PPRZ;
